...
export async function getReportSnapshot(): Promise<ReportSnapshot> {
  ...
  const byCashier = await db.getAllAsync<{ name: string; total: number; gross: number; voids: number; refunds: number; count: number }>(`SELECT u.name, COALESCE(SUM(CASE WHEN s.status = 'completed' THEN s.total ELSE 0 END), 0) AS total, COALESCE(SUM(CASE WHEN s.status IN ('completed','voided','refunded','partially_refunded') THEN s.total ELSE 0 END), 0) AS gross, COALESCE(SUM(CASE WHEN s.status = 'voided' THEN s.total ELSE 0 END), 0) AS voids, COALESCE(SUM(CASE WHEN s.status IN ('refunded','partially_refunded') THEN s.total ELSE 0 END), 0) AS refunds, SUM(CASE WHEN s.status = 'completed' THEN 1 ELSE 0 END) AS count FROM users u LEFT JOIN sales s ON s.cashier_id = u.id AND s.deleted_at IS NULL WHERE u.deleted_at IS NULL GROUP BY u.id ORDER BY CASE u.role WHEN 'admin' THEN 0 ELSE 1 END, u.name`);
